#include "stdafx.h"
#include "SmoothNormalFromDepth.h"
...
megamol::compositing::SmoothNormalFromDepth::SmoothNormalFromDepth()
        : m_version(0)
    ...
    this->m_output_tex_slot.SetCallback(CallTexture2D::ClassName(), "GetData", &SmoothNormalFromDepth::getDataCallback);
    this->m_output_tex_slot.SetCallback(
        CallTexture2D::ClassName(), "GetMetaData", &SmoothNormalFromDepth::getMetaDataCallback);
...
bool megamol::compositing::SmoothNormalFromDepth::create() {
    vislib::graphics::gl::ShaderSource compute_shader_src;

    GetCoreInstance()->ShaderSourceFactory().MakeShaderSource("Compositing::smoothNormalFromDepth", compute_shader_src);
    ...
    try {
        m_smooth_normal_from_depth_prgm = std::make_unique<glowl::GLSLProgram>(shader_srcs);
    } catch (glowl::GLSLProgramException const& exc) {
        megamol::core::utility::log::Log::DefaultLog.WriteError(
            "Error during shader program creation of\"%s\": %s. [%s, %s, line %d]\n", "Compositing::smoothNormalFromDepth",
            exc.what(), __FILE__, __FUNCTION__, __LINE__);
    }

    glowl::TextureLayout tx_layout(GL_RGBA16F, 1, 1, 1, GL_RGBA, GL_HALF_FLOAT, 1);
    m_output_texture = std::make_shared<glowl::Texture2D>("smooth_normal_from_depth_output", tx_layout, nullptr);

    return true;
}

void megamol::compositing::SmoothNormalFromDepth::release() {}

bool megamol::compositing::SmoothNormalFromDepth::getDataCallback(core::Call& caller) {
    ...
    if (something_has_changed) {
        ...
        m_smooth_normal_from_depth_prgm->use();

        auto inv_view_mx = glm::inverse(view_mx);
        auto inv_proj_mx = glm::inverse(proj_mx);
        m_smooth_normal_from_depth_prgm->setUniform("inv_view_mx", inv_view_mx);
        m_smooth_normal_from_depth_prgm->setUniform("inv_proj_mx", inv_proj_mx);

        glActiveTexture(GL_TEXTURE0);
        input_tx2D->bindTexture();
        m_smooth_normal_from_depth_prgm->setUniform("src_tx2D", 0);
...
bool megamol::compositing::SmoothNormalFromDepth::getMetaDataCallback(core::Call& caller) {
